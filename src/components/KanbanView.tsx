            ...
            <div key={column.id} className="w-72 flex flex-col">
              ...
              <div className="flex items-center justify-between mb-3 px-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-openclaw-textMuted bg-openclaw-bg px-2 py-0.5 rounded-full">
                    {getTasksByColumn(column.id).length}
                  </span>
                </div>
                <button className="p-1 rounded hover:bg-openclaw-cardHover text-openclaw-textMuted">
              ...
              <div className="flex-1 space-y-2 min-h-[200px]">
                {getTasksByColumn(column.id).map(task => (
                  <motion.div
                    ...
                    className="glass rounded-lg p-3 border border-openclaw-border hover:border-openclaw-primary/30 transition-all cursor-grab active:cursor-grabbing group"
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button 
                            ...
                            className="p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-openclaw-error/20 text-openclaw-textMuted hover:text-openclaw-error transition-all"
                          >
                        ...
                        {task.description && (
                          <p className="text-xs text-openclaw-textMuted mt-1 line-clamp-2">{task.description}</p>
                        )}

                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          <span className={`px-2 py-0.5 rounded text-xs ${priorityColor(task.priority)}`}>
                            {task.priority}
                          </span>
                          {task.tags.map(tag => (
                            <span key={tag} className="px-2 py-0.5 rounded text-xs bg-openclaw-bg border border-openclaw-border text-openclaw-textMuted">
                              {tag}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center gap-1 text-xs text-openclaw-textMuted">
                ...
                {newTaskColumn === column.id ? (
                  <div className="glass rounded-lg p-3 border border-openclaw-primary">
                    ...
                    <div className="flex gap-2 mt-2">
                      <button
                        ...
                        className="flex-1 px-2 py-1 rounded bg-openclaw-primary text-white text-xs"
                      >
                        Adicionar
                      </button>
                      <button
                        ...
                        className="px-2 py-1 rounded bg-openclaw-bg text-openclaw-textMuted text-xs"
                      >
                        Cancelar
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full p-2 rounded-lg border border-dashed border-openclaw-border text-openclaw-textMuted hover:border-openclaw-primary hover:text-openclaw-primary transition-colors text-sm flex items-center justify-center gap-1"
                  >
