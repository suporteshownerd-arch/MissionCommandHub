  ...
  return (
    <div className="h-full flex gap-4">
      <div className="w-80 flex flex-col glass rounded-xl overflow-hidden">
        <div className="p-4 border-b border-openclaw-border">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-openclaw-text flex items-center gap-2"><Bot className="w-5 h-5 text-openclaw-primary" />Agentes</h2>
            <button className="p-1.5 rounded-lg bg-openclaw-primary hover:bg-openclaw-primaryHover text-white transition-colors shadow-glow-primary hover:shadow-[0_0_25px_rgba(255,92,92,0.5)]"><Plus className="w-4 h-4" /></button>
          </div>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-openclaw-textMuted" />
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar agentes..." className="w-full bg-openclaw-bg border border-openclaw-border rounded-lg pl-9 py-2 text-sm text-openclaw-text placeholder-openclaw-textMuted focus:outline-none focus:border-openclaw-primary focus:ring-1 focus:ring-openclaw-primary/30 transition-all" />
          </div>
        </div>

        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          {loading ? (
            <div className="flex items-center justify-center p-8"><Loader className="w-6 h-6 animate-spin text-openclaw-primary" /></div>
          ) : filteredAgents.map((agent) => (
            <button
              key={agent.id}
              onClick={() => setSelectedAgent(agent.id)}
              className={`w-full text-left p-3 rounded-lg border transition-all ${selectedAgent === agent.id ? 'bg-openclaw-primary/15 border-openclaw-primary/50 shadow-glow-primary' : 'bg-openclaw-bgHover border-openclaw-border hover:border-openclaw-primary/30 hover:bg-openclaw-cardHover'}`}
            >
              <div className="flex items-start justify-between">
                ...
                <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 mt-1.5 ${agent.status === 'active' ? 'bg-openclaw-success shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-openclaw-textMuted'}`} />
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 glass rounded-xl overflow-hidden flex flex-col">
        {selectedAgent ? <AgentDetail agent={agents.find(a => a.id === selectedAgent)} /> : (
          <div className="flex-1 flex flex-col items-center justify-center text-openclaw-textMuted">
            <div className="w-20 h-20 rounded-2xl bg-openclaw-bg flex items-center justify-center mb-4 border border-openclaw-border"><Bot className="w-10 h-10 opacity-30" /></div>
    ...
    <>
      <div className="p-5 border-b border-openclaw-border flex items-center justify-between bg-openclaw-bg/50">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-xl bg-openclaw-primary/20 flex items-center justify-center shadow-glow-primary"><span className="text-3xl">{agent.icon || '🤖'}</span></div>
          <div>
        ...
        <div className="flex items-center gap-2">
          <button className="px-4 py-2 rounded-lg bg-openclaw-success/20 text-openclaw-success hover:bg-openclaw-success/30 transition-colors flex items-center gap-2 text-sm font-medium"><Play className="w-4 h-4" />Iniciar</button>
          <button className="p-2 rounded-lg hover:bg-openclaw-bgHover text-openclaw-textMuted hover:text-openclaw-text transition-colors"><Edit className="w-4 h-4" /></button>
          <button className="p-2 rounded-lg hover:bg-openclaw-bgHover text-openclaw-textMuted hover:text-openclaw-error transition-colors"><Trash2 className="w-4 h-4" /></button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6">
        <div>
          ...
          <div className="flex items-center gap-3 px-4 py-3 bg-openclaw-bg rounded-lg border border-openclaw-border">
            ...
            {agent.status === 'active' && <span className="ml-auto badge badge-success">Executando</span>}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-3 px-4 py-3 bg-openclaw-bg rounded-lg border border-openclaw-border">
            ...
            <button className="p-1.5 rounded hover:bg-openclaw-cardHover text-openclaw-textMuted hover:text-openclaw-text transition-colors"><ExternalLink className="w-4 h-4" /></button>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {['Pesquisa', 'Análise', 'Automação', 'Integração'].map((cap) => <span key={cap} className="px-3 py-1.5 rounded-lg bg-openclaw-bg border border-openclaw-border text-openclaw-textMuted text-sm hover:border-openclaw-primary/50 hover:text-openclaw-primary transition-colors cursor-pointer">{cap}</span>)}
          </div>
        </div>
