...
export default function MonitorView() {
  ...
  const stats: StatCard[] = [
    { label: 'Gateway', value: status.connected ? 'Online' : 'Offline', icon: status.connected ? Wifi : WifiOff, color: status.connected ? 'text-openclaw-success' : 'text-openclaw-error', bgColor: status.connected ? 'bg-openclaw-success/10' : 'bg-openclaw-error/10' },
    ...
    <div className="h-full overflow-y-auto space-y-6 pb-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-openclaw-textMuted">Última atualização: {lastUpdate.toLocaleTimeString('pt-BR')}</p>
        </div>
        <button onClick={refresh} disabled={loading} className="p-2 rounded-lg bg-openclaw-card border border-openclaw-border text-openclaw-textMuted hover:text-openclaw-primary hover:border-openclaw-primary/50 transition-colors disabled:opacity-50">
      ...
      <div className="glass rounded-xl p-5 border">
        ...
        {loading ? (
          <div className="flex items-center justify-center p-8"><RefreshCw className="w-6 h-6 animate-spin text-openclaw-primary" /></div>
        ) : agents.length > 0 ? (
          <div className="space-y-3">
            {agents.map((agent) => {
              const statusInfo = getAgentStatus(agent.status)
              return (
                <div key={agent.id} className="flex items-center justify-between p-3 rounded-lg bg-openclaw-bg border border-openclaw-border hover:border-openclaw-primary/30 transition-colors">
      ...
      <div className="glass rounded-xl p-5 border">
        ...
        <div className="bg-openclaw-bg rounded-lg border border-openclaw-border p-3 font-mono text-xs space-y-1 max-h-48 overflow-y-auto">
          ...
          {status.connected ? <div className="text-openclaw-success">[2026-03-28 01:25:03] Gateway conectado com sucesso</div> : <div className="text-openclaw-error">[2026-03-28 01:25:03] Falha ao conectar ao Gateway</div>}
