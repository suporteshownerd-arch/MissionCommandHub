...
export default function MCPPanel() {
  ...
  const handleExecute = async () => {
    ...
    if (selectedAgent) {
      await executeAgent(selectedAgent, command)
    } else {
      addLog(`⚠️ Selecione um agente primeiro`)
    }
    setCommand('')
  }
            ...
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-colors ${
                activeTab === tab 
                  ...
                  : 'text-openclaw-textMuted hover:bg-openclaw-bgHover hover:text-openclaw-text'
              }`}
            >
        ...
        <div className="flex-1 glass rounded-xl overflow-hidden">
          <AnimatePresence mode="wait">
            {activeTab === 'agents' && (
              ...
              >
                <div className="p-3 border-b border-openclaw-border">
                  <h3 className="font-medium text-openclaw-text flex items-center gap-2">
                    <Bot className="w-4 h-4 text-openclaw-primary" />
                    Agentes MCP
                  </h3>
                </div>
                    ...
                    <motion.button
                      ...
                      className={`w-full p-3 rounded-lg border transition-all text-left ${
                        selectedAgent === agent.id
                          ...
                          : 'bg-openclaw-bgHover border-openclaw-border hover:border-openclaw-primary/30'
                      }`}
                    >
                      ...
                      {agent.currentTask && (
                        <div className="mt-2 text-xs text-openclaw-primary truncate">
                          → {agent.currentTask}
                        </div>
                      )}
                    </motion.button>
            ...
            {activeTab === 'tools' && (
              ...
              >
                <div className="p-3 border-b border-openclaw-border">
                  <h3 className="font-medium text-openclaw-text flex items-center gap-2">
                    <Wrench className="w-4 h-4 text-openclaw-accent" />
                    Ferramentas MCP
                  </h3>
                </div>
                  ...
                  {mcpTools.map((tool) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full p-2.5 rounded-lg border border-openclaw-border hover:border-openclaw-primary/30 hover:bg-openclaw-bgHover transition-all text-left flex items-center gap-3 group"
                      >
                        <div className={`p-2 rounded-lg bg-openclaw-bg ${colorClass}`}>
                          <Icon className="w-4 h-4" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-openclaw-text">{tool.name}</span>
                            <span className="text-xs px-1.5 py-0.5 rounded bg-openclaw-bg text-openclaw-textMuted capitalize">
                              {tool.category}
                            </span>
                          </div>
            ...
            {activeTab === 'console' && (
              ...
              >
                <div className="p-3 border-b border-openclaw-border flex items-center justify-between">
                  <h3 className="font-medium text-openclaw-text flex items-center gap-2">
                    <Terminal className="w-4 h-4 text-openclaw-warning" />
                    Console de Comandos
                  </h3>
                  <button 
                    onClick={() => addLog('🗑️ Console limpo')}
                    className="text-xs text-openclaw-textMuted hover:text-openclaw-text"
                  >
                    Limpar
                  </button>
                </div>
                <div className="flex-1 overflow-y-auto p-3 space-y-1 font-mono text-xs">
                  {logs.length === 0 ? (
                    <p className="text-openclaw-textMuted text-center py-4">
                      Nenhuma atividade ainda...
                    </p>
                  ) : (
                    logs.map((log, i) => (
                      <div key={i} className="text-openclaw-textMuted hover:text-openclaw-text">
                        {log}
                      </div>
                    ))
                  )}
      ...
      <div className="flex-1 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-openclaw-textMuted">
                    {selectedAgentData.status === 'idle' ? 'Aguardando comando' :
                     selectedAgentData.status === 'thinking' ? 'Pensando...' :
                     selectedAgentData.status === 'running' ? `Executando: ${selectedAgentData.currentTask}` :
                     'Erro'}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {selectedAgentData.status === 'running' ? (
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-openclaw-error/20 text-openclaw-error hover:bg-openclaw-error/30 transition-colors flex items-center gap-2"
                  >
                  ...
                  <button
                    onClick={() => executeAgent(selectedAgentData.id, 'Iniciar')}
                    className="px-4 py-2 rounded-lg bg-openclaw-success/20 text-openclaw-success hover:bg-openclaw-success/30 transition-colors flex items-center gap-2"
                  >
              ...
              <div className="flex flex-wrap gap-2">
                {selectedAgentData.tools.map((toolId) => {
                  ...
                  return tool ? (
                    <span 
                      key={tool.id}
                      className="px-2 py-1 rounded bg-openclaw-bg border border-openclaw-border text-xs text-openclaw-textMuted"
                    >
                      {tool.icon} {tool.name}
                    </span>
                  ) : null
        ...
        <div className="glass rounded-xl p-4 border flex-1 flex flex-col">
          <div className="flex items-center gap-2 mb-3">
            <Terminal className="w-4 h-4 text-openclaw-warning" />
            <span className="text-sm font-medium text-openclaw-text">Digite um comando</span>
          </div>
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            {commandSuggestions.slice(0, 6).map((suggestion) => (
              <button
                ...
                onClick={() => setCommand(suggestion.example)}
                className="text-xs px-2 py-1 rounded bg-openclaw-bg border border-openclaw-border text-openclaw-textMuted hover:border-openclaw-primary/50 hover:text-openclaw-primary transition-colors"
              >
                {suggestion.command}
              </button>
            ))}
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg bg-openclaw-primary hover:bg-openclaw-primaryHover text-white transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-glow-primary"
              >
