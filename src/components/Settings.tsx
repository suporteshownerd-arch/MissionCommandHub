...
export default function Settings({ isOpen, onClose }: SettingsProps) {
  ...
  const testConnection = async () => {
    ...
    try {
      const res = await fetch(`${settings.openclawUrl}/health`, {
        method: 'GET',
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="glass rounded-xl border w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden"
        >
          {/* Header */}
          <div className="p-4 border-b border-openclaw-border flex items-center justify-between">
            <h2 className="text-lg font-semibold text-openclaw-text flex items-center gap-2">
              <Shield className="w-5 h-5 text-openclaw-primary" />
              Configurações
            </h2>
            <button
              onClick={onClose}
              ...
              {tabs.map(tab => (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${
                    activeTab === tab.id
                      ? 'bg-openclaw-primary/15 text-openclaw-primary'
              ...
              {activeTab === 'general' && (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 rounded-lg bg-openclaw-bg border border-openclaw-border">
                    <div>
                      <p className="text-sm text-openclaw-text font-medium">Auto-refresh</p>
                      <p className="text-xs text-openclaw-textMuted">Atualizar dados automaticamente</p>
                    </div>
                    <button
                      onClick={() => setSettings(s => ({ ...s, autoRefresh: !s.autoRefresh }))}
                      className={`w-11 h-6 rounded-full transition-colors ${
                        settings.autoRefresh ? 'bg-openclaw-primary' : 'bg-openclaw-border'
                      }`}
                    >
                  ...
                  {settings.autoRefresh && (
                    <div className="p-3 rounded-lg bg-openclaw-bg border border-openclaw-border">
                      ...
                      <input
                        ...
                        className="w-full accent-openclaw-primary"
                      />
                    </div>
                  )}
                </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={e => setSettings(s => ({ ...s, openclawUrl: e.target.value }))}
                          placeholder="http://localhost:3456"
                          className="flex-1 bg-openclaw-bg border border-openclaw-border rounded-lg px-3 py-2 text-sm text-openclaw-text placeholder-openclaw-textMuted focus:outline-none focus:border-openclaw-primary"
                        />
                        <button
                          onClick={testConnection}
                          disabled={testingApi}
                          className="px-3 py-2 rounded-lg bg-openclaw-primary/20 text-openclaw-primary hover:bg-openclaw-primary/30 transition-colors text-sm flex items-center gap-2"
                        >
                          {testingApi ? <RefreshCw className="w-4 h-4 animate-spin" /> : <ExternalLink className="w-4 h-4" />}
                          Testar
                        </button>
                      </div>
                      {apiStatus !== 'idle' && (
                        <div className={`flex items-center gap-2 mt-2 text-sm ${apiStatus === 'success' ? 'text-openclaw-success' : 'text-openclaw-error'}`}>
                          {apiStatus === 'success' ? <Check className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                          {apiStatus === 'success' ? 'Conexão estabelecida!' : 'Falha na conexão'}
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-openclaw-bg border border-openclaw-border rounded-lg px-3 py-2 text-sm text-openclaw-text font-mono"
                      />
                    </div>
              ...
              {activeTab === 'appearance' && (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className={`p-3 rounded-lg border transition-all flex items-center gap-2 ${
                          settings.theme === 'dark'
                            ? 'border-openclaw-primary bg-openclaw-primary/10 text-openclaw-primary'
                            : 'border-openclaw-border text-openclaw-textMuted hover:border-openclaw-textMuted'
                        }`}
                      >
                    ...
                    <p className="text-xs text-openclaw-textMuted mt-2">
                      Tema light em breve • Por enquanto apenas dark mode
                    </p>
                  </div>
                </div>
              )}
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={item.key} className="flex items-center justify-between p-3 rounded-lg bg-openclaw-bg border border-openclaw-border">
                        <div>
                          <p className="text-sm text-openclaw-text font-medium">{item.label}</p>
                          <p className="text-xs text-openclaw-textMuted">{item.desc}</p>
                        </div>
                        <button
                          ...
                          className={`w-11 h-6 rounded-full transition-colors ${
                            settings.notifications[item.key as keyof typeof settings.notifications] ? 'bg-openclaw-primary' : 'bg-openclaw-border'
                          }`}
                        >
                          <div className={`w-5 h-5 rounded-full bg-white transition-transform ${
          ...
          <div className="p-4 border-t border-openclaw-border flex items-center justify-between">
            <div className="text-xs text-openclaw-textMuted">
              v1.0.0 • Mission Command Hub
            </div>
            <div className="flex items-center gap-2">
              {saved && (
                <span className="text-sm text-openclaw-success flex items-center gap-1">
              ...
              <button
                onClick={handleSave}
                className="px-4 py-2 rounded-lg bg-openclaw-primary hover:bg-openclaw-primaryHover text-white transition-colors text-sm font-medium"
              >
                Salvar
              </button>
            </div>
